...
(define (make-table-with-key key)
  (cons key (cons undefined-value (make-empty-tree))))
(define (table-key table)
  (car table))
(define (table-root-value table)
  (car (cdr table)))
(define (table-subtable-tree table)
  (cdr (cdr table)))
(define (set-table-root-value! table new-root-value)
  (let ((value-cell (cdr table)))
    (set-car! value-cell new-root-value)))
(define (add-subtable! table subtable comp-fn)
  (let ((subtable-tree (table-subtable-tree table)))
    (insert-tree-item! subtable subtable-tree table-key comp-fn)
    'ok))
...
(println "(table-subtable-tree rt): " (table-subtable-tree rt))
(define rst (make-table-with-key 0))
(set-table-root-value! rst 'zero)
(add-subtable! rt rst num-comp)
(println "added <0> → 'zero")
rt
(println "(table-subtable-tree rt): " (table-subtable-tree rt))

(define (make-table key-comparison-fn)
  (let ((local-table 
          (make-table-with-key '*table*)))
    (define (find-record! creating-if-needed rest-of-key-path subtable)
      (if (null? rest-of-key-path) 
          subtable
          (let ((nested-subtable (find-tree-item 
                                   (car rest-of-key-path)
                                   (table-subtable-tree subtable)
                                   table-key
                                   key-comparison-fn)))
            (if nested-subtable
